...
lines = [97.3, 186.2, 276.7]
positions = [15.5, 145.6, 274.8]
blackLineDown = [8.3, 10, 10.9]  # A VERIF ?
blackLineUp = [85.8, 87.4, 88.4]  # A VERIF ?
...
gridPath = scriptPath + "grille_blm.jpg"

texts = {'titre': {'position': (width/2, height-10*mm), 'font-size': 16, 'font': defaultFont},
         'pagination': {'position': (width-15*mm, height-10*mm), 'font-size': 16, 'font': defaultFont},
         'rappel-sequence': {'position': (width-31*mm, height-10*mm), 'font-size': 13, 'font': defaultFont},
         'sous-titre': {'position': (width-147*mm, height-10*mm), 'font-size': 11, 'font': defaultFont},
...
metas = {'action': {'text-align': 'center', 'position': (imgWidth/2, 4), 'multiple-lines': True, 'ecart': 5, 'lines-up': False, 'font-size':fontSize, 'font': defaultFont},
         'dialogue': {'text-align': 'center', 'position': (imgWidth/2, -imgHeigth-3), 'multiple-lines': True, 'ecart': 5, 'lines-up': True, 'font-size':fontSize, 'font': defaultFont},
         'orientation': {'text-align': 'left', 'position': (2, 4), 'multiple-lines': True, 'ecart': 5, 'lines-up': False, 'font-size':8, 'font': defaultFont},
         'echelle': {'text-align': 'right', 'position': (imgWidth-5, 8), 'multiple-lines': False, 'ecart': 0, 'lines-up': False, 'font-size':8, 'font': defaultFont},
...
for l in f.readlines():
    ...
    if l:
        ...
        if l == "." or l == "SAUT DE PAGE":
            t = len(thumbnails)
            p = int(t/cases_x * cases_y)
            r = cases_x * cases_y-abs(p*cases_x * cases_y - t)
            for i in range(0, r):
            ...
            p = int(t/cases_x * cases_y)
            r = cases_x * cases_y-abs(p*cases_x * cases_y - t)
            if r != cases_x * cases_y:
                for i in range(0, r):
                    thumbnails.append(newThumbnail())
            # Remplir la page actuelle

            for i in range(0, cases_x * cases_y):
                thumbnails.append(newThumbnail())
...
def newPage(c, pageNumber, pages, customBackground=None, justtext=False):
    ...
    c.setFont(texts['sous-titre']['font'], texts['sous-titre']['font-size'])
    c.drawString(texts['sous-titre']['position'][0], texts['sous-titre']['position'][1], subTitle)
...
pages = int(len(thumbnails)/(cases_x * cases_y))
if len(thumbnails) % cases_x * cases_y:
...
for k, t in enumerate(thumbnails):
    ...
    if (line == cases_y - 1 and position == cases_x) or page == 0:
        # Fin de la 4e position de la 3e ligne : nouvelle page
        page += 1
        position = 0
        line = 0
        customBackground = None
        startIndex = (page - 1) * (cases_x * cases_y)
        endIndex = page * (cases_x * cases_y)
    ...
    if name in ["-", "-/", "CASE VIDE", "CASE VIDE/"]:
        ...
        if not t['fullpage']:
            ...
            c.drawString((positions[position]+2)*mm, height-(lines[line]-imgHeigth-7)*mm, "%s" % texte.lower())
